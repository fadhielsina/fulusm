...
function cekData() {
	var tbl = $('#tblDetail');
	var isValidateAkun = true;
	var lastRow = tbl.find("tr").length;
	var debitSum = 0;
	var kreditSum = 0;
	var akunExist = new Array();

	if ( $('#tblDetail').data('disable-validate-akun') ) {
		isValidateAkun = false;
	}
...
function addRow() {
	...
	if (emptyrows == 0 ) {
		...
		ddlAkun += $("#akun1").html();
		ddlAkun += '</select>';
		var txtDebit = '<input name="debit'+lastRow+'" id="debit'+lastRow+'" class="form-control form-control-line" title="Debit harus diisi dengan angka" onBlur="cekDebit('+lastRow+');" />';
		var txtKredit = '<input name="kredit'+lastRow+'" id="kredit'+lastRow+'" class="form-control form-control-line" title="Kredit harus diisi dengan angka" onBlur="cekKredit('+lastRow+');" />';
