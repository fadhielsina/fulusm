...
class Barang_sales extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('barang_sales_model');	
		$this->load->model('barang_model');	
	}
	
	function index()
	{
		$data['title'] = "Display Barang Jual";
		$data['main_content'] = 'barang_sales/display';
		$data['client_data'] = $this->barang_sales_model->get_all_data();
		$this->load->view('template/template_xpanel', $data);
	}

	function add()
	{
		$data['title'] = "Tambah Barang Jual";
		$data['main_content'] = 'barang_sales/form';
		$data['act'] = 'add';
		$data['kategori_data'] = $this->barang_sales_model->get_kategori_data();
	...
	function view()
	{
		...
		$data['title'] = "Lihat barang Jual";
		$data['main_content'] = 'barang_sales/form';
		$data['act'] = 'view';
		$data['form_act'] = '';
		$data['kategori_data'] = $this->barang_sales_model->get_kategori_data();
		$data['client_data'] = $this->barang_sales_model->get_data_by_id($id);
		$this->load->view('template/template_xpanel', $data);
	}	
	...
	{
		$id = $this->uri->segment(3);
		$data['title'] = "Edit barang Jual";
		$data['main_content'] = 'barang_sales/form_edit';
		$data['satuan1'] = $this->barang_model->get_satuan();
		$data['satuan2'] = $this->barang_model->get_satuan();
		$data['act'] = 'edit';
	...
	function insert()
	{
		$this->barang_sales_model->fill_data();
		$this->barang_sales_model->insert_data();
		$this->session->set_userdata('SUCCESSMSG', 'Input Barang baru sukses ;)');
		redirect('barang_sales');			
	}

	function update()
	{
		$id = $this->uri->segment(3);
		$this->barang_sales_model->fill_data_up();
		$this->barang_sales_model->update_data($id);
