...
class Fulusme extends REST_Controller
{
    ...
    function paymentt_post()
    {
        ...
        $cek_va = $this->db->select('*')->from('virtual_account_copy')
            ->where('virtual_account', $va_no)
            ->order_by('id', 'DESC')
            ->limit(1)->get();
        ...
        if ($cek_va->num_rows() < 1) :
            ...
            $this->db->insert('va_payment_copy', $data);
            $last_id = $this->db->insert_id();
            ...
            // $this->db->update('va_payment_copy', $data_response, ['id' => $last_id]);
            // $this->db->update('va_payment_copy', ['response_code' => '000101'], ['id' => $last_id]);
            // $this->db->query("UPDATE va_inquiry_copy SET response_code = '000101' WHERE va_no = '$va_no' AND bill_amount = '$bill_amount' ORDER BY id DESC LIMIT 1");
            // $this->db->where('id', $id_va)->update('virtual_account_copy', ['response_code' => '000101']);
            $this->update_pendanaan($cek_va->row());
            $this->response($data_response, REST_Controller::HTTP_OK);
        endif;
    }
    ...
    function update_pendanaan($data_va)
    {
        ...
        $cek_pendanaan = $this->db_fulus->select('*')->from('pendanaan_copy')
        ...
        if ($cek_pendanaan->num_rows() > 0) :
            if (!$cek_pendanaan->row()->paid_ts || $cek_pendanaan->row()->paid_ts == 0) :
                $this->db_fulus->update('pendanaan_copy', ['status' => 1, 'paid_ts' => $time1], ['id' => $pendanaan_id]);
                ...
                $danaTerkumpul = $this->db->get_where('trx_project_copy', ['id_project' => $id_project])->row()->dana_terkumpul;
                ...
                if (!$cek) {
                    ...
                    $this->db->update('trx_project_copy', ['dana_terkumpul' => $update_dana]);
