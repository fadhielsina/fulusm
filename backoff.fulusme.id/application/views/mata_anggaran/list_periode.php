	...
	<table cellpadding="0" cellspacing="0" border="0" class="display" id="display_table">
		...
		<tbody>
			...
			foreach ($anggaran as $key => $value) {
				...
				echo form_button(array('id' => 'button-edit','class' => 'btn btn-sm waves-effect waves-light btn-rounded btn-primary', 'content' => $this->lang->line('aktifkan'), 'onClick' => "document.location.href='"
					.$action_aktifkan."/".$value[$id]."'"));
				}else{
					echo form_button(array('id' => 'button-delete','class' => 'btn btn-sm waves-effect waves-light btn-rounded btn-success', 'content' => $this->lang->line('aktif')));
					echo form_button(array('id' => 'button-edit','class' => 'btn btn-sm waves-effect waves-light btn-rounded btn-primary', 'content' => $this->lang->line('non_aktifkan'), 'onClick' => "document.location.href='"
					.$action_aktifkan."/".$value[$id]."/0'"));
				}
