...
?>
	...
	<div class="buttons pull-right">
		<?php 
			echo form_button(array('id' => 'button-addnew','class' => 'btn btn-sm waves-effect waves-light btn-rounded btn-success', 'content' => 'Tambah Baru', 'onClick' => "location.href='".site_url()."hutang/add_saldo'" ));
				...
				if($piutang_data)
				{
					...
					foreach ($piutang_data as $row)
					{
					    ...
					    echo '<td>'.number_format($row->bayar).'</td>';
						 echo '<td>'.number_format($row->trxTotal-$row->bayar).'</td>';
						echo '</tr>';
					}
				}
			?>	
