...
<div class="pull-right">
		<?php 
			echo form_button(array('id' => 'button-addnew','class' => 'btn btn-sm waves-effect waves-light btn-rounded btn-success', 'content' => $this->lang->line('tambah'), 'onClick' => "location.href='".site_url()."kas/add_kas_in'" ));
			//echo anchor(site_url().'kas/add_kas_setor_bank', 'Setor Kas', 'class="btn btn-sm waves-effect waves-light btn-rounded btn-warning"');
		?>		
	</div>
</div>
<br/><br/>
				...
				render: function( data, type, row ){
					html = '<div class="btn-group">'+
							...
							'<div class="dropdown-menu">' +
								'<a class="dropdown-item" href="<?php echo site_url('kas/detail_data_in'); ?>/'+ row.id_trx +'">Detail</a>';
					if ( row.no ) {
						html += '<a class="dropdown-item" href="<?php echo site_url('kas/nota'); ?>/'+ row.id_trx +'" target="_blank">Nota</a>';
						var conf_label = "'<?= $this->lang->line('valid_hapus') ?> : "+ row.id_trx +" ?'";
						html += '<a class="dropdown-item" href="<?php echo site_url('kas/delete_data_kas'); ?>/'+ row.id_trx +'/km" onclick="javascript: return confirm('+ conf_label +')"><?= $this->lang->line('hapus') ?></a>';
					}
					html += '</div></div>';
					return html;
				}
