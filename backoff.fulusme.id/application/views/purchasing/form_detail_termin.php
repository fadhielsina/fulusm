...
<?php foreach ($invoice_data as $row) { $stp=$row->status_piutang; $receivableID=$row->receivableID;$is_pending=$row->is_pending;$noinv=$row->invoiceID;$totalinv=$row->trxTotal;$totalbyar=$row->totalbyr;$userinput=$row->nama_depan;$is_publish=$row->is_publish;$sisa=$row->trxTotal-$row->totalbyr;}?>
...
<?php if($stp!=='2') { ?>
  ...
  <?php echo form_close(); ?>
<?php } else { echo "<HR/><div class='col-lg-4'><h4 style='border:dashed #000 thin;padding:8px;background:#d1d1d1;border-radius:8px;'>INVOICE INI SUDAH LUNAS</h4></div>";?>
