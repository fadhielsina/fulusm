	...
	<?php if ( $data ): ?>
  	<div class="card card-outline-info">
		  ...
		  <div class="card-body">
			...
			<table id="example23" cellpadding="0" cellspacing="0" border="0" class="display">
				<thead>
					<tr>
			 ...
			 buttons: [
	            { extend: 'print', footer: true,
	            	customize: function ( win ) {
	            		var $header = $('<?php echo $print_header; ?>'),
	            			$body = $(win.document.body);

	            		$body.find('h1').replaceWith( $header );
	            		$body.find('table td').css({
	            			borderBottom: '1px solid #000',
	            			borderTop: '0px'
	            		});
	            		$body.find('table th').css({
	            			borderTop: '1px solid #000'
	            		}).end().find('table th:first-child, table td:first-child').css({
	            			borderLeft:'1px solid #000'
	            		}).end().find('table th, table td').css({
	            ...
	            { extend: 'pdfHtml5', footer: true,
	            	customize: function( doc ){
	            		doc.content[0].stack = headerStack;
	            	} }
	        ],
			"bJQueryUI": true,
