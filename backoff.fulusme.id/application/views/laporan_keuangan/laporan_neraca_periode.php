			...
			<div class="card-body">
				...
				<?php echo form_open( $form_action, array( 'id' => 'form_laporan' ) ); ?>	
					...
					<div class="form-group row m-b-10">
						<label class="col-sm-2 col-form-label">Dari Periode</label>
						<div class="col-sm-3 tampil-tahun-hide">
							<?php 
								$datad['id'] = 'start_bulan';
								$datad['class'] = "form-control";					
								$selected = date("m");
								echo form_dropdown('start_bulan', $months, $selected, $datad);
							?>
						</div>
						<div class="col-sm-3">
								<?php 
					...
					<div class="form-group row">
						<label class="col-sm-2 col-form-label">s/d Periode</label>
						<div class="col-sm-3 tampil-tahun-hide">
							<?php 
								$datad['id'] = 'end_bulan';
								$datad['class'] = "form-control";					
								$selected = date("m");
								echo form_dropdown('end_bulan', $months, $selected, $datad);
							?>
						</div>
						<div class="col-sm-3">
								<?php 
									$datad['id'] = 'end_tahun';
									$selected = date("Y") ;
									echo form_dropdown('end_tahun', $years, $selected, $datad);
								?>
						</div>
					</div>
					...
					<div class="form-group custom-checkbox">
                        <div>
                          <input type="hidden" name="tampil_data_tahunan" value="0">
                          <input class="form-check-input" type="checkbox" value="1" name="tampil_data_tahunan" id="tampil_data_tahunan">
                          <label class="form-check-label" for="tampil_data_tahunan">
                            Tampilkan periode tahunan
                          </label>
                        </div>
                    </div>
...
<script type="text/javascript">
	$( function() {
	    ...
	    $('#tampil_data_tahunan').on('change', function(e){
	    	if ( this.checked ) {
	    		$('.tampil-tahun-hide').hide();
	    	} else {
	    		$('.tampil-tahun-hide').show();
	    	}
	    });
	} );
</script>