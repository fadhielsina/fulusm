...
class Data_model extends CI_Model
{
	...
	{
		return $this->db_fulus->select('*, user_id as id_user')->get('peminjam')->result();
	}

	function get_all_data_pendana()
	{
		return $this->db_fulus->select('*, id as id_user, full_name as name')->get('pendana')->result();
	}
	...
	{
		$this->db_fulus->select('peminjam.*, perusahaan_peminjam.*, user.email as email');
...
// 		$config['upload_path'] = '../public_html/assets/file_user/prospektus/';
	...
	function postApprove($id)
	{
		...
		$dataTrx = [
			'tgl_app' => date('y-m-d'),
			'status' => 1,
			'tgl_deadline' => date('Y-m-d', strtotime("+60 day", strtotime(date('y-m-d')))),
			'uang_muka' => $uang_muka
		];

		$data = [
			'status' => 1,
			'end_ts' => strtotime('+60 day')
		];
