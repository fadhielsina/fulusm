...
class Laporan extends CI_Controller
{
	...
	function laporan_keuangan()
	{
		...
		if ($data['jenis_laporan'] == "neraca") {
			...
			$data['dataLaporan'] = $dataOperasional;
			$dataOps;
			...
			$data['dataLaporan'] = $this->db->query(
				"SELECT 
			...
			$data['dataLaporanS'] = $this->db->query(
				"SELECT 
				'Kas_Arus' header,'Arus kas Dari Aktivitas Operasi' as grouping, akun.nama as nama_akun, akun.*, jurnal_detail.debit_kredit
				FROM `akun` 
				LEFT JOIN jurnal_detail ON jurnal_detail.akun_id = akun.id 
	...
	public function lb_standard()
	{
		...
		if ('post' === $this->input->method()) {
			...
			$show_all = " AND akun_saldo.identityID=" . $this->identity_id . " AND jurnal.identityID=" . $this->identity_id;
			...
			$data['data'] = $this->db->query("SELECT 'kegiatan Operasional' header,'Pendapatan' as grouping, akun.*,
				...
				FROM (`akun` 
				INNER JOIN akun_saldo ON akun_saldo.akun_id = akun.id )
				left JOIN jurnal_detail ON jurnal_detail.akun_id = akun.id 
					AND jurnal_detail.akun_id  = akun_saldo.akun_id 
				...
				FROM `akun` 
				LEFT JOIN akun_saldo ON akun_saldo.akun_id = akun.id 
				left JOIN jurnal_detail ON jurnal_detail.akun_id = akun.id LEFT JOIN jurnal on jurnal.id = jurnal_detail.jurnal_id AND jurnal.tgl >='" . $start_qr . "' AND jurnal.tgl <='" . $end_qr . "'
				WHERE kelompok_akun_id=6
				" . $show_all . "
	...
	public function lb_berjalan()
	{
		$id = $this->session->userdata('SESS_USER_ID');
		$data['title'] = 'Laba/Rugi (Periode Berjalan)';
		$data['main_content'] = 'laporan_keuangan/laporan_laba_rugi_periode_berjalan';
		...
		if ('post' === $this->input->method()) {
			...
			$data['date_periodes'] = $this->_date_range($start_date, $end_date);
			$query_tgl = array();
		...
		if ('post' === $this->input->method()) {
			...
			$data['data'] = $this->db->query("SELECT 'kegiatan Operasional' header,'Pendapatan' as grouping, akun.*,sum(if(debit_kredit=0,nilai,0)) kredit, sum(if(debit_kredit=1,nilai,0)) debit, " . $query_print . " 
				...
				INNER JOIN akun_saldo ON akun_saldo.akun_id = akun.id )
				 left JOIN jurnal_detail ON jurnal_detail.akun_id = akun.id 
				 AND jurnal_detail.akun_id  = akun_saldo.akun_id 
				 LEFT JOIN jurnal on jurnal.id = jurnal_detail.jurnal_id AND YEAR(jurnal.tgl) >=" . $data['start_tahun'] . " AND YEAR(jurnal.tgl) <= " . $data['end_tahun'] . "
				WHERE kelompok_akun_id=4 
				AND kode not like '45%'
				" . $show_all . "
				GROUP BY akun.kode
	...
	public function neraca()
	{
		...
		if ('post' === $this->input->method()) {
			...
			$data['data'] = $this->db->query("
				...
				WHERE kelompok_akun_id=1 and kode REGEXP '^(1|2|3)' AND length(kode) > 3 " . $show_all . " GROUP BY akun.kode
				UNION
				...
				left JOIN jurnal_detail ON jurnal_detail.akun_id = akun.id 
				LEFT JOIN jurnal on jurnal.id = jurnal_detail.jurnal_id AND jurnal.identityID = akun_saldo.identityID 
				WHERE kode REGEXP '^(50|5|22)' " . $show_all . " GROUP BY akun.kode
				UNION
	...
	public function arus_kas()
	{
		...
		if ('post' === $this->input->method()) {
			...
			$end_date = sprintf('%d-%d-01', $this->input->post('end_tahun'), $this->input->post('end_bulan'));
			$this->identity_id = $this->input->post('lokasi');
			$show_all = " AND akun_saldo.identityID=" . $this->identity_id;
			...
			WHERE kode REGEXP '^(9|8)' AND jurnal.identityID=" . $this->identity_id . " 
			" . $show_all . "
			GROUP BY akun.kode
			UNION									
			SELECT 
			...
			AND jurnal_detail.akun_id  = akun_saldo.akun_id
			LEFT JOIN jurnal ON jurnal.id = jurnal_detail.jurnal_id AND jurnal.tgl >='" . $start_date . "' AND jurnal.tgl <= LAST_DAY('" . $end_date . "')
			WHERE kode REGEXP '^(4)' AND jurnal.identityID=" . $this->identity_id . " 
			" . $show_all . "
			GROUP BY akun.kode
			UNION
			SELECT 
			...
			AND jurnal_detail.akun_id  = akun_saldo.akun_id
			LEFT JOIN jurnal ON jurnal.id = jurnal_detail.jurnal_id AND jurnal.tgl >='" . $start_date . "' AND jurnal.tgl <= LAST_DAY('" . $end_date . "') 
			WHERE kode REGEXP '^(6)' AND jurnal.identityID=" . $this->identity_id . " 
			" . $show_all . "
			GROUP BY akun.kode
	...
	public function ekuitas()
	{
		...
		if ('post' === $this->input->method()) {
			...
			$data['data'] = $this->db->query("
				...
				AND jurnal_detail.akun_id  = akun_saldo.akun_id
				LEFT JOIN jurnal on jurnal.id = jurnal_detail.jurnal_id AND jurnal.tgl >='" . $start_qr . "' AND jurnal.tgl <='" . $end_qr . "' 
				WHERE kelompok_akun_id=2 AND jurnal.identityID=" . $this->identity_id . " 
				" . $show_all . "
				GROUP BY akun.kode
				UNION
				...
				AND jurnal_detail.akun_id  = akun_saldo.akun_id
				LEFT JOIN jurnal on jurnal.id = jurnal_detail.jurnal_id AND jurnal.tgl >='" . $start_qr . "' AND jurnal.tgl <='" . $end_qr . "'
				WHERE kelompok_akun_id=3 AND jurnal.identityID=" . $this->identity_id . " 
				" . $show_all . "
				GROUP BY akun.kode
				UNION
				...
				AND jurnal_detail.akun_id  = akun_saldo.akun_id
				LEFT JOIN jurnal on jurnal.id = jurnal_detail.jurnal_id AND jurnal.tgl >='" . $start_qr . "' AND jurnal.tgl <='" . $end_qr . "'
				WHERE kelompok_akun_id IN (4,5) AND jurnal.identityID=" . $this->identity_id . "
				" . $show_all . "
				GROUP BY akun.kode
				")->result_array();
...

	{
		...
		$data['jenis_laporan'] = "jatuh tempo";

		$data['main_content'] = 'laporan_jatuh_tempo/form';
...

	{
		...
		$data['dataLaporan'] = "laporan_jatuh_tempo/form";
		...
		if ($data['jenis_laporan'] == 'jatuh tempo') {

			$data['project'] = $this->db->query('SELECT * FROM trx_jatuh_tempo

												WHERE tgl_tempo LIKE "' . $filter . '" ')->result();

			$this->load->view('laporan_jatuh_tempo/form_laporan', $data);
		} else {
