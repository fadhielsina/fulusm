...
require APPPATH . '/libraries/REST_CI_Controller.php';

/**
 * Keys CI_Controller
 * This is a basic Key Management REST CI_Controller to make and delete keys
 *
 * @package         CodeIgniter
 ...
 */
class Key extends REST_CI_Controller {
    ...
    public function index_put()
    {
        ...
        if ($this->_insert_key($key, ['level' => $level, 'ignore_limits' => $ignore_limits]))
        {
            $this->response([
                'status' => TRUE,
                'key' => $key
            ], REST_CI_Controller::HTTP_CREATED); // CREATED (201) being the HTTP response code
        }
        else
        {
            $this->response([
            ...
            ], REST_CI_Controller::HTTP_INTERNAL_SERVER_ERROR); // INTERNAL_SERVER_ERROR (500) being the HTTP response code
        }
    }
    ...
    public function index_delete()
    {
        ...
        if (!$this->_key_exists($key))
        {
            ...
            ], REST_CI_Controller::HTTP_BAD_REQUEST); // BAD_REQUEST (400) being the HTTP response code
        }
        ...
        $this->response([
            ...
            ], REST_CI_Controller::HTTP_NO_CONTENT); // NO_CONTENT (204) being the HTTP response code
    }
    ...
    public function level_post()
    {
        ...
        if ($this->_update_key($key, ['level' => $new_level]))
        {
            ...
            ], REST_CI_Controller::HTTP_OK); // OK (200) being the HTTP response code
        }
        else
        {
            $this->response([
