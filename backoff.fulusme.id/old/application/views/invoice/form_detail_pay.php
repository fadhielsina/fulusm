...
<?php foreach ($invoice_data as $row) { $stp=$row->isfinish;$is_pending=$row->is_pending;$noinv=$row->invoiceID;$totalinv=$row->trxTotal;$totalbyar=$row->totalbyr+$row->trxPay;$amount=$row->amount;$userinput=$row->nama_depan;$is_publish=$row->is_publish;$sisa=$row->trxTotal-$totalbyar;}?>
	
<div class="post-body">
<?php if($stp!=='1') { ?>
	<?php echo form_open('invoice/pending_pay_data', array('id' => 'jurnal_form')); ?>	
  ...
  <div class="panel-body">
	<table width="300" border="0" align="center" cellpadding="2" cellspacing="0">
		...
		<input type="checkbox" name="stsbyr" value="2" > Lunas
		<hr/>
		</td>
		</tr>
		  <tr>
  ...
  <?php echo form_close(); ?>
  <?php } else { echo "<h5 class='pull-right' style='border:dashed #000 thin;padding:8px;background:#FFDC73;border-radius:8px;width:20%'>INVOICE INI SUDAH LUNAS</h5>";?>
...
<table cellpadding="0" cellspacing="0" border="0" >
	...
	<tbody>
	<?php if($invoice_data) { 
	foreach ($invoice_data as $row1) 
				{ 
				if($row1->trxPay!='0' || $row1->trxPay!=null || $row1->trxPay!=' ') {
					echo '<tr>';
					echo '<td>'.$row->trxDate.'</td>';
					echo '<td>'.$row->nm_tipe.'</td>';
					echo '<td>-</td>';
					echo '<td>'.number_format($row->trxPay).'</td>';		echo '</tr>';
	?>
		...
		<?php 
			if($invoice_data_detail)
			{
				...
				{ 
					...
					echo '<td>'.number_format($row->amount).'</td>';		echo '</tr>';
				}
			}
		?>
	</tbody>	
...
<?php if($stp!=='1') { ?>
  <hr/>
<div class="col-lg-12"><br/><br/><h4>JURNAL INVOICE PENDING</h4></div>
