...
?>
  ...
  <div class="card-body">
    <div class="pull-right">
		<?php 
			echo form_button(array('id' => 'button-addnew','class' => 'btn btn-xs waves-effect waves-light btn-success',  'content' => 'Kembali', 'onClick' => "location.href='".site_url()."pembelian/buku_pemb'" ));
		?>		
	</div>
...
<table class="table" id="display_table_pembelian">
				...
				if($account_data)
				{
					...
					foreach ($account_data as $row)
					{
						$edit_data = array(
							...
							'barangBarcode' => $row->barangBarcode,
							'barangName' => $row->barangName,
							'barangID' => $row->barangID,
							'satuanID' => $row->satuanID,
							'qty' => $row->detailBuyQty,
							'satuan_1' => $row->detailBuySat1,
							'qty_2' => $row->detailBuyQty2,
							'satuan_2' => $row->detailBuySat2,
							'harga' => $row->detailBuyPrice,
							'diskon' => $row->discPercent
						);
						$edit_data = htmlspecialchars(json_encode($edit_data), ENT_QUOTES, 'UTF-8');
						echo '<tr>';
						...
						echo '<td>'.$row->detailBuyQty.' ('.$row->sat1.') </td>';
						echo '<td>'.$row->detailBuyQty2.' ('.$row->sat2.') </td>';
						echo '<td>'.number_format($row->discPercent).'</td>';
						echo '<td>'.number_format( $row->detailBuySubtotal ).'</td>';
						echo '<td class="no-print"><a href="#" data-options="'.$edit_data.'" class="btn btn-warning btn-xs btn-edit-barang">Edit</a></td>';
						echo '</tr>';
		...
		<tfoot>
			<tr>
				<td colspan="7"></td><td>TOTAL</td><td><b><?php echo number_format($total); ?></b></td><td></td>
			</tr>
		</tfoot>
	</table>
	</div>
