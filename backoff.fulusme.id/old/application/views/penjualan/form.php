					...
					<div class="row">
						<div class="col">
							...
							<table id="table_pembelian" class="table table-bordered table-pembelian">
								...
								<tbody>
									<td colspan="7" class="notif-empty">Tidak ada data yang diinputkan</td>
								</tbody>
							</table>
						</div>
					</div>
            ...
            <form id="pembelian_form">
                ...
                <div class="modal-body">
					...
					<div class="form-group row">
						...
						<div class="col-sm-3">
							<select name="satuan_1" id="satuan_1" class="form-control" style="width: 100%;" required="">
...
<script>
	...
	$('#pembelian_form').on('submit', function(event){
		...
		var data = $(this).serializeObject(),
			...
			$template = $('<tr class="tr-detail-'+ index +'" data-index="'+ index +'"><td>'+ data.kode +'</td><td><input type="hidden" name="detail['+ index +'][idbarang]" value="' + data.kode + '"> '+ data.nama +'</td>' +
						...
						'<td>' +
							...
							'<input type="hidden" name="detail['+ index +'][subtotal]" value="'+ ( data.qty*(data.harga-data.diskon )+(0.1*(data.qty*(data.harga-data.diskon )))) +'">'+
	...
	$('body').on('click', '.edit-detail-item--js', function( event ){
		...
		models[ index ]['subtotal'] = parseInt( ( ( models[ index ]['harga'] - models[ index ]['diskon'] ) * models[ index ]['qty'] ) );
		...
		submodels['total'] = submodels['subtotal'] + submodels['pajak'];
	...
	function termin_opt() {
		...
		if (idTermin == 2) {
			myDiv.html("<div class='form-group row'><label class='col-sm-3 col-form-label'>Jatuh Tempo</label><div class='col'><input type='text' class='form-control' id='tgl_termin' name='tgl_termin'></div></div>")
		} else {
	...
	function load_jurnal_otomatis() {
		
		var nama_jurnal = 'pembelian_cash';
		if ( submodels['pajak'] > 0 || submodels['diskon'] > 0 ) {
			nama_jurnal = 'pembelian_cash_diskon_pajak';
		}

		if ( $('#termin').val() == 2 ) {
			nama_jurnal = 'pembelian_termin';
			if ( submodels['pajak'] > 0 || submodels['diskon'] > 0 ) {
				nama_jurnal = 'pembelian_termin_diskon_pajak';
			}
		}

		console.log(nama_jurnal, 'nama_jurnal');

		var data = $('#find-template-select option[data-name="'+ nama_jurnal +'"]').val();
		load_jurnal( data );
	}

	function load_jurnal( data ) {
		var tbl = $('#tblDetail');
