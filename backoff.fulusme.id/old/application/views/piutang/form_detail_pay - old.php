<div class="post-body">
	<?php echo form_open('piutang/insert_pay_recv'); ?>
	<div class="col-lg-12">
		<div class="card card-outline-info">
			<div class="card-header">
				<h4 class="mb-0 text-white pull-left">Pembayaran Piutang</h4><span class="pull-right">
				<a id="btn_lihat_bayar" class="btn btn-success btn-sm" data="<?php echo $memberID; ?>"><span style="color:#fff;font-weight:bolder;">Histori Pembayaran</span></a>
				<a href="<?php echo site_url().'piutang/kartu_piutang/'.$memberID; ?>" class="btn btn-primary btn-sm" ><span style="color:#fff;font-weight:bolder;"> Kartu Piutang</span></a>
				</span>
			</div>
			<div class="card-body">
				<div class="container">
					<div class="row">
							...
							<label class="col-lg-12 col-form-label">Customer</label>
									<input type="text" name="no_po" class="form-control text-uppercase" value="<?php echo $member; ?>" disabled >
									<input type="hidden" name="memberID" id="memberID" class="form-control text-uppercase" value="<?php echo $memberID; ?>" >
								<input type="hidden" name="identity" id="identity" class="form-control text-uppercase" value="<?php echo $identity; ?>" >
								</div>
								<div class="col">
								<label class="col-lg-12 col-form-label">No. Bayar(Otomatis)</label>
					...
					<div class="col">
								<label class="col-lg-12 col-form-label">Memo</label><br/>
									<input type="text" name="memo" class="form-control text-uppercase" value="Penerimaan Piutang Dari <?php echo $member; ?>" >
								</div>
						</div>
					<div class="row">
						<div class="col">
            ...
            <form class="form-horizontal">
                <div class="modal-body">
					...
					<input name="debtID" id="debtID" class="form-control" type="hidden" style="width:335px;" readonly>
					 <div class="form-group">
                        <label class="control-label col-xs-3" >NO SJ / Faktur</label>
                        <div class="col-xs-9">
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        function tampil_data_barang(){
            ...
            $.ajax({
				type : "POST",
                url   : '<?php echo site_url('auto/ajax_get_cus') ?>',
        ...
        $('#show_data').on('click','.item_edit',function(){
            ...
            $.ajax({
               	type : "GET",
                url   : '<?php echo site_url('auto/ajax_get_cus_id') ?>',
                dataType : "JSON",
                data : {id:id},
                success: function(data){
                    $.each(data,function( note, trxTotal){
						...
						$('[name="debtID"]').val(data.receivableID);
                    });
                }
            });
            return false;
         ...
         function tampil_riwayat_bayar(){
            ...
            $.ajax({
				type : "POST",
                url   : '<?php echo site_url('auto/ajax_get_recv_pay') ?>',
				data : {memberID:memberID},
                async : true,
                dataType : 'json',
                success : function(data){
                    ...
                    var i;
					  var totalbyr=0;
                    for(i=0; i<data.length; i++){
						totalbyr+=parseInt(data[i].receivablePay);
                        html += '<tr>'+
                                '<td>'+data[i].note+'</td>'+
                                '<td>'+data[i].receivableDate+'</td>'+
                                '<td>'+formatRupiah(data[i].receivablePay)+'</td>'+
                                '</tr>';
                    }
        ...
        $('#btn_simpan').on('click',function(){
            ...
            $.ajax({
                type : "POST",
                 url   : '<?php echo site_url('piutang/simpan_termin_pay') ?>',
                dataType : "JSON",
